...
class DenseRetriever(BaseRetriever):
    def __init__(self, config):
        super().__init__(config)
        import sys
        print(f"[RETRIEVER] Loading FAISS index from: {self.index_path}", file=sys.stderr, flush=True)
        self.index = faiss.read_index(self.index_path)
        print(f"[RETRIEVER] ✓ Index loaded: {self.index.ntotal} vectors", file=sys.stderr, flush=True)
        
        if config.faiss_gpu:
            print(f"[RETRIEVER] Moving {self.index.ntotal} vectors to GPU (may take 10-20 min)...", file=sys.stderr, flush=True)
            co = faiss.GpuMultipleClonerOptions()
            co.useFloat16 = True
            co.shard = True
            self.index = faiss.index_cpu_to_all_gpus(self.index, co=co)
            print("[RETRIEVER] ✓ Index moved to GPU", file=sys.stderr, flush=True)

        print(f"[RETRIEVER] Loading corpus from: {self.corpus_path}", file=sys.stderr, flush=True)
        self.corpus = load_corpus(self.corpus_path)
        print(f"[RETRIEVER] ✓ Corpus loaded: {len(self.corpus)} documents", file=sys.stderr, flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=8000, help="Port to run the server on")  # NEW
    ...
    # 3) Launch the server with configurable port
    uvicorn.run(app, host="0.0.0.0", port=args.port, log_level="warning")  # CHANGED
